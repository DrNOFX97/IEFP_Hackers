...
<body>
    ...
    <div id="auth-gate">
        <div class="auth-card">
            ...
            <div class="auth-sub">Acesso reservado a participantes do curso.<br>Inicia sessão com a tua conta institucional.</div>
            <button class="auth-btn" onclick="signInWithMicrosoft()">
                ...
                Entrar com conta IEFP
            </button>
            <div style="display:flex;align-items:center;gap:0.5rem;margin:0.2rem 0 0.5rem;">
                <div style="flex:1;height:1px;background:var(--border-color);"></div>
                <span style="font-size:0.72rem;color:var(--text-secondary);">ou</span>
                <div style="flex:1;height:1px;background:var(--border-color);"></div>
            </div>
...
`);
        ...
        const MS_LOGO = '<svg width="18" height="18" viewBox="0 0 21 21" style="vertical-align:middle;margin-right:8px"><rect x="1" y="1" width="9" height="9" fill="#f25022"/><rect x="11" y="1" width="9" height="9" fill="#7fba00"/><rect x="1" y="11" width="9" height="9" fill="#00a4ef"/><rect x="11" y="11" width="9" height="9" fill="#ffb900"/></svg> Entrar com conta IEFP';

        function signInWithMicrosoft() {{
            const btn = document.querySelector('.auth-btn');
            const provider = new firebase.auth.OAuthProvider('microsoft.com');
            provider.setCustomParameters({{ prompt: 'select_account', tenant: 'formacao.iefp.pt' }});
            if (btn) {{ btn.disabled = true; btn.style.opacity = '0.6'; btn.textContent = 'A entrar…'; }}
            auth.signInWithPopup(provider).catch(e => {{
                if (e.code === 'auth/popup-blocked' || e.code === 'auth/popup-closed-by-user') {{
                    auth.signInWithRedirect(provider);
                }} else {{
                    if (btn) {{ btn.disabled = false; btn.style.opacity = ''; btn.innerHTML = MS_LOGO; }}
