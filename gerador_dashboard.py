...
def build_pg_examples():
    ...
    return [
        ...
        ex('🔍 Mini-Scanner (CTF)', [
            '# ── Mini-Scanner de Reconhecimento ─────────────────────',
            '# Simula um scanner DNS + port scan + relatório.',
            '# Corre este script para obteres a flag do CTF.',
            '',
            'import socket, hashlib, datetime',
            '',
            'ALVO = "pentestlab.local"',
            'PORTAS = [21, 22, 23, 25, 53, 80, 110, 443, 3306, 8080]',
            '',
            '# ── DNS (simulado em sandbox) ───────────────────────────',
            'print(f"[*] Alvo: {ALVO}")',
            'print(f"[*] DNS  → 192.168.56.10  (simulado)")',
            '',
            '# ── Port scan ───────────────────────────────────────────',
            'abertas = [22, 80, 443]   # portas "abertas" simuladas',
            'print("[*] Port scan:")',
            'for p in PORTAS:',
            '    estado = "ABERTA  ◀" if p in abertas else "fechada"',
            '    print(f"    {p:5d}/tcp  {estado}")',
            '',
            '# ── Relatório ───────────────────────────────────────────',
            'ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")',
            'relatorio = f"report_{ALVO}_{ts}.txt"',
            'print(f"[*] Relatório guardado em: {relatorio}")',
            '',
            '# ── Flag de conclusão ───────────────────────────────────',
            'seed = (ALVO + "recon_done").encode()',
            'digest = hashlib.sha256(seed).hexdigest()[:8].upper()',
            'print()',
            'print("=" * 45)',
            'print(f"  flag{{py_recon_script_done}}")',
            'print("=" * 45)',
        ]),
        ex('🏗️ Classes (OOP)', [
